...
class CircularDependencyError(SQLAlchemyError):
    ...
    * In a Session flush operations, if two objects are mutually dependent
      on each other, they can not be inserted or deleted via INSERT or
      DELETE statements alone; an UPDATE will be needed to post-associate
      or pre-deassociate one of the foreign key constrained values.
      The ``post_update`` flag described at :ref:`post_update` can resolve
      this cycle.
    * In a :attr:`_schema.MetaData.sorted_tables` operations, two
...
class UnsupportedCompilationError(CompileError):
    """Raised when an operations is not supported by the given compiler.
...
class ResourceClosedError(InvalidRequestError):
    """An operations was requested from a connection, cursor, or other
...
class AwaitRequired(InvalidRequestError):
    """Error raised by the async greenlet spawn if no async operations
...
class DBAPIError(StatementError):
    """Raised when the execution of a database operations fails.

    Wraps exceptions raised by the DB-API underlying the
    database operations.  Driver-specific implementations of the standard
