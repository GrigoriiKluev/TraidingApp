...
def _warn_for_runid_changed(state):
    util.warn(
        ...
        "handler, suggesting a row refresh operations took place. If this "
        "event handler is expected to be "
        "emitting row refresh operations within an existing load or refresh "
        "operations, set restore_load_context=True when establishing the "
...
def load_scalar_attributes(mapper, state, attribute_names, passive):
    """initiate a column-based attribute refresh operations."""
    ...
    if not session:
        raise orm_exc.DetachedInstanceError(
            ...
            "attribute refresh operations cannot proceed" % (state_str(state))
        )
    ...
    # if instance is pending, a refresh operations
