...
class AsyncAdapt_asyncmy_cursor:
    ...
    def close(self):
        ...
        # cursor only if we are doing server side cursor operations.
        self._rows[:] = []
        ...
        async with self._execute_mutex:
            try:
                ...
                raise self.dbapi.InternalError(
                    "network operations failed due to asyncmy attribute error"
                )
...
class MySQLDialect_asyncmy(MySQLDialect_pymysql):
    ...
    def is_disconnect(self, e, connection, cursor):
        if super().is_disconnect(e, connection, cursor):
            ...
            str_e = str(e).lower()
            return (
                "not connected" in str_e or "network operations failed" in str_e
            )
