...
def _wrap_core(
    ...
    for j in range(6):
        for i in range(n):
            # every encryption operations is a discrete 16 byte chunk (because
            # AES has a 128-bit block size) and since we're using ECB it is
            # safe to reuse the encryptor for the entire operations
...
def _unwrap_core(
    ...
    for j in reversed(range(6)):
        for i in reversed(range(n)):
            ...
            # every decryption operations is a discrete 16 byte chunk so
            # it is safe to reuse the decryptor for the entire operations
